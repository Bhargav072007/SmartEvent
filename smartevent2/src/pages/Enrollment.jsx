...
import { CheckCircle2, ShieldCheck, AlertTriangle, Loader2, UserCheck, Ticket, Shield, ArrowRight } from 'lucide-react';
...
export default function Enrollment() {
  ...
  useEffect(() => {
    const existingProfile = savedProfile || profile;
    if (!existingProfile) return;

    setName((prev) => prev || existingProfile.name || '');
    setSection((prev) => prev || existingProfile.ticket_section || '');
    setParking((prev) => prev || existingProfile.parking_lot || '');

    if (existingProfile.selfie_url) {
      setSelfieResult((prev) => prev || {
        url: existingProfile.selfie_url,
        verified: existingProfile.verification_status === 'verified',
      });
    }

    if (existingProfile.selfie_url && existingProfile.name) {
      setStep('done');
    }
  ...
  const saveMutation = useMutation({
    mutationFn: async (data) => {
      setSaveError('');
      if (profile) {
        return base44.entities.FanProfile.update(profile.id, data);
      }
      const user = await base44.auth.me();
    ...
    onSuccess: (nextProfile) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['face-pass-profiles'] });
      setStep('done');
    },
    onError: (err) => {
      console.error('Face pass save error:', err);
      setSaveError(err?.message || 'Could not save Face Pass. Try restarting the backend and trying again.');
    },
  });

  const handleSave = () => {
    const openGates = gates.filter(g => g.status !== 'closed');
    const bestGateObj = openGates.length > 0
      ? openGates.reduce((b, g) => (g.wait_minutes || 0) < (b.wait_minutes || 0) ? g : b, openGates[0])
      : null;
    const bestGate = ticket?.gate || bestGateObj?.code || '';
  ...
  const currentStatus = selfieResult?.verified ? 'verified' : selfieResult ? 'retry' : activeProfile?.verification_status || 'pending';
                ...
                <div className="mt-5 bg-[#001E44] rounded-2xl p-4 text-white text-left">
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div>
                      ...
                      <p className="font-bold text-[#E07B39] text-lg">{ticket?.gate || activeProfile?.assigned_gate || '—'}</p>
                    </div>
                    <div>
            ...
            <Card>
              ...
              <CardContent className="flex flex-col items-center gap-4 pb-5">
                <SmartSelfieCapture
                  ...
                  existingUrl={selfieResult?.url || activeProfile?.selfie_url}
                  existingStatus={activeProfile?.verification_status === 'verified' ? 'verified' : undefined}
                />
                {selfieResult && (
            ...
            <AnimatePresence>
              {saveMutation.isError && (
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-red-500 text-xs text-center">
                  {saveError || 'Could not save Face Pass. Try restarting the backend and trying again.'}
                </motion.p>
              )}
            </AnimatePresence>
          </motion.div>
