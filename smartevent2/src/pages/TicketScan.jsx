...
import { QrCode, Ticket, CheckCircle2, Loader2, ArrowRight, ChevronRight, Upload, Camera, ArrowLeftRight, UserCheck } from 'lucide-react';
import { base44 } from '@/api/base44Client';
import { buildTicketQRPayload, findTicketRecord, parseTicketQRPayload, registerTicketRecord, seedTicketRecords } from '@/lib/ticketRegistry';
...
const qrUrlForTicket = (ticket, size = 220) => {
...
async function decodeQRCodeFromFile(file) {
  ...
  return payload?.[0]?.symbol?.[0]?.data || null;
}
...
function QRUploadPanel({ onResult, scanning }) {
  ...
  const handleFile = async (file) => {
    ...
    setAnalyzing(true);
    try {
      const rawValue = await decodeQRCodeFromFile(file);
      const parsed = parseTicketQRPayload(rawValue);
      if (!parsed?.code) {
        setError('QR could not be read on this device. Try manual code entry or use Chrome for the demo.');
        setAnalyzing(false);
        return;
      }
      setAnalyzing(false);
      onResult(parsed.code, parsed.payload);
    } catch (scanError) {
      console.error('QR decode error:', scanError);
      setError('QR scan failed. Try a clearer screenshot or use manual entry.');
      setAnalyzing(false);
    }
  };
...
export default function TicketScan({ onTicketScanned }) {
  ...
  React.useEffect(() => {
    seedTicketRecords(
      Object.entries(MOCK_TICKETS).map(([code, ticket]) => ({ ...ticket, code }))
    );
  }, []);

  const handleScan = async (ticketCode, scannedPayload = null) => {
    const c = (ticketCode || code).trim().toUpperCase();
    if (!c) return;
    setScanning(true);
    setError('');
    await new Promise(r => setTimeout(r, 900));
    const storedTicket = findTicketRecord(c);
    const ticket = storedTicket || MOCK_TICKETS[c] || null;
    if (!ticket) {
      setError('Ticket not found. Try: DEMO, PSU-2024-A1234, PSU-2024-B5678');
      setScanning(false);
      return;
    }
    const finalTicket = registerTicketRecord({
      ...ticket,
      ...scannedPayload,
      code: c,
    });
  ...
  const handleQRResult = (ticketCode, payload) => {
    handleScan(ticketCode, payload);
  };
  ...
  const handleConfirm = async () => {
    ...
    const enrichedTicket = registerTicketRecord({
      ...found,
      holderName: user?.name || found.holderName,
      email: user?.email || found.email,
    });
    onTicketScanned(enrichedTicket);
  };
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === 'confirming' && found && (
            <motion.div key="confirm" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="space-y-5">
              ...
              <div className="bg-white border border-[#001E44]/10 rounded-2xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Entry QR</p>
                    <p className="text-sm text-[#001E44] font-bold">Show this at Gate {found.gate}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground">Seat</p>
                    <p className="text-sm font-semibold">{found.section} · {found.row} · {found.seat}</p>
                  </div>
                </div>
                <div className="flex justify-center">
                  <img
                    src={qrUrlForTicket(found)}
                    alt="Ticket QR code"
                    className="w-56 h-56 rounded-xl border border-gray-200 bg-white p-2"
                  />
                </div>
                <p className="text-xs text-center text-muted-foreground font-mono">{found.code || 'DEMO'}</p>
              </div>
